...
for index,current_face_location in enumerate(all_face_locations):
    ...
    #convert above image to 3d numpy array
    img_pixels = image.img_to_array(current_face_image)
    #expand it to single row of multiple column
    img_pixels = np.expand_dims(img_pixels, axis = 0)
   #normalize all pixel from 0 to 1
    img_pixels /= 255 
    #doing predection
    exp_predictions = face_exp_model.predict(img_pixels) 
   #finding max index predection
    max_index = np.argmax(exp_predictions[0])
    #getting label for detected emotion
    emotion_label = emotions_label[max_index]
    
    #display the name as text in the image
    font = cv2.FONT_HERSHEY_DUPLEX
    cv2.putText(image_to_detect, gender+" "+age+"yrs"+" "+emotion_label, (left_pos,bottom_pos+20), font, 0.5, (0,255,0),1)
cv2.imshow("Age-Gender and Emotion detection",image_to_detect)
